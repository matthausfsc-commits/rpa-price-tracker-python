...
def executar_rpa_pro():
    ...
    if not pdfs:
        print("⚠️  AVISO: Coloca os teus PDFs na pasta 'catalogos' e corre o código novamente.")
        return

    # 2. Processamento em Massa (Mês 1: Loops)
    for arquivo in pdfs:
        caminho_completo = os.path.join(PASTA_CATALOGOS, arquivo)
        print(f"📄 A analisar: {arquivo}")
        
        paginas = contar_paginas_pdf(caminho_completo)
        
        dados_finais.append({
            "Nome do Arquivo": arquivo,
            "Total de Páginas": paginas,
            "Data da Extração": datetime.now().strftime("%d/%m/%Y %H:%M")
        })

    # 3. Exportação e Inteligência (Mês 3: Pandas)
    df = pd.DataFrame(dados_finais)
    df.to_excel(ARQUIVO_EXCEL, index=False)
    
    print(f"\n✅ SUCESSO: Relatório gerado em: {ARQUIVO_EXCEL}")
    print(f"📊 Total de ficheiros processados: {len(pdfs)}")
